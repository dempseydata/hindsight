...
- Day buckets follow the Mac's clock zone. `HINDSIGHT_TZ=America/New_York` (any IANA name) pins them to a zone instead — for a travelling laptop, or a "my day" that is not the machine's; export it in the shell that runs `serve.py`, and before `analyze.py install` so the nightly carries it. An unknown name warns once and uses the host zone. Charts re-bucket on the next reload; each session's stored start day catches up at the next analysis run, so the ledger can sit a day off its bars until then.
