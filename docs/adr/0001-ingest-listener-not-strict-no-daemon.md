...
A menu bar icon needs a process that lives as long as the login session, which the boundary test forbids Hindsight from owning. The same reasoning that admitted the nightly job admits it: the host is a third-party app the operator installs, SwiftBar, and Hindsight ships only the script it runs every 30 s, `build/menubar.py` (issue #37), reached through a two-line wrapper `install` writes into SwiftBar's plugin folder with the interpreter baked in, as the plists do. The script reads `GET /health` on the listener — the one GET it answers, added for this and touching no data — and the open `breakage` rows read-only; its menu items call `launchctl` and `analyze.py acknowledge-breakage`, the commands the operator would otherwise type. Its Start for the views server launches the same on-demand `serve.py`, detached from a terminal but with no plist and no `KeepAlive`: it lives until stopped or logout, which is a foreground process without a window, not a daemon. Nothing new runs in the background and nothing new writes.
