...
4. **Reader and writer share a DB with no WAL and no reader `busy_timeout`.** A nightly run holding an exclusive lock makes `serve.py` raise an unhandled `OperationalError` → stdlib 500 (`serve.py:85, 607-611`). Nothing corrupts; the page just fails until the write finishes.
...
7. **Extracts and raw model output live on disk in plain text** under `local-data/analysis/` — user and assistant message text at 1,500 chars per piece, plus `analyze.log` echoing the first 80 chars of any rejected model output (`analyze.py:621`). Gitignored; not encrypted.
