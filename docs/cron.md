...
| Menu bar plugin `hindsight.30s.sh` | every 30 s while SwiftBar is running (a third-party host, ADR-0001 amendment 2026-09-23) | wrapper → `<sys.executable> build/menubar.py`: `GET /health`, `breakage` read-only (`mode=ro`); `GET /` on the views server; menu items run `menubar.py start|stop` (`launchctl` on the ingest plist), `menubar.py serve|unserve` (`serve.py` detached to `~/Library/Logs/hindsight/serve.log`; `SIGTERM` to whatever holds `:8321`) and `analyze.py acknowledge-breakage <id>` | none | 2 s probe timeout; DB absent → "no open breakages"; table unreadable → shown in red, never read as clean | none — the next tick re-reads |
