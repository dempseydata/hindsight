...
Two launchd user agents, one Claude Code hook and one SwiftBar plugin. `serve.py` is a foreground process with no job of its own; there is no other daemon (ADR-0001). Nothing here holds a secret: the nightly's model calls go through the operator's own `claude` login.
...
| Menu bar plugin `hindsight.30s.sh` | every 30 s while SwiftBar is running (a third-party host, ADR-0001 amendment 2026-09-23) | wrapper → `<sys.executable> build/menubar.py`: `GET /health`, `breakage` read-only (`mode=ro`); `GET /` on the views server; menu items run `menubar.py start|stop` (`launchctl` on the ingest plist), `menubar.py serve|unserve` (`serve.py` detached to `~/Library/Logs/hindsight/serve.log`; `SIGTERM` to whatever holds `:8321`), `menubar.py open` (serve if down, then `open` the what view) and `analyze.py acknowledge-breakage <id>` | none | 2 s probe timeout; DB absent → "no open breakages"; table unreadable → shown in red, never read as clean | none — the next tick re-reads |
...
Install / remove: `python3 build/listener.py install|uninstall`, `python3 build/analyze.py install|uninstall`, `python3 build/menubar.py install|uninstall` (the last writes a two-line wrapper into SwiftBar's plugin folder, read from its `PluginDirectory` preference and set to `~/Library/Application Support/SwiftBar/Plugins` if unset; the wrapper bakes `sys.executable` and the script path like the plists). Both write the plist under `~/Library/LaunchAgents/`, `launchctl bootout` (failure ignored) then `bootstrap gui/<uid>`. Plists bake install-time absolutes — re-run after moving the clone or changing Python. Inspect with `launchctl print gui/$(id -u)/com.hindsight.nightly`.
