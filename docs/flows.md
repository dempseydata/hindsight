...
| --- | --- | --- | --- |
| 1. `sync_sessions` — glob `~/.claude/projects/*/*.jsonl` | `analyze.py:461-475` | Head-scan first 3,000 chars for one of four hindsight prompt signatures → `excluded_sessions`, never analysed (ADR-0003). Subagent transcripts (`*/*/subagents/*.jsonl`) are not synced as sessions — the substrate scan reaches them through their parent (ADR-0019) | INSERT `sessions` (status `pending`) |
...
| 3. `invalidate_grown` | `analyze.py:487-533` | Skips live transcripts (mtime < 300 s, parent or any subagent transcript). Growth = parent larger than `sessions.size`, or a subagent transcript unseen/larger than `subagent_transcripts.size`. Re-audit only if *parent* growth ≥ 25 % of `audited_size` (ADR-0019: subagents never re-audit) | DELETE substrate rows, unlink extract + cache files, status → `pending` |
| 4. `fill_substrate` | `substrate.py` | `skipped_records IS NULL` watermark; live guard. Scans the parent and its subagent transcripts in one pass, `agent_id` on each row (ADR-0019); counts every key per record version into the field histogram (issue #15) | INSERT `tool_events`, `usage`, `command_grains`, `subagent_transcripts`, `field_histogram`; UPDATE `audit.adr_count` |
| 4a. `check_drift` | `analyze.py` | Newest record version with ≥20 records vs the nearest earlier one, against `substrate.FIELD_CONTRACT`; this run's skipped share vs the median of the last 7 runs (ADR-0020). Never halts the run | INSERT `breakage` (deduplicated against open rows), `scan_runs`; `osascript` notification for the thin-rows conditions when run from the command line |
