...
| 4. Respond | `listener.py:195-201` | `200` for any accepted body. `GET /health` → `200 {"ok": true}`, any other GET → `404` (#37, the menu bar probe; no DB touch) | stderr → `~/Library/Logs/hindsight/listener.log` (exception text and skip counts; never bodies) |
