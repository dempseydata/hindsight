...
| 4. `fill_substrate` | `substrate.py:216-232` | `skipped_records IS NULL` watermark; live guard. Scans the parent and its subagent transcripts in one pass, `agent_id` on each row (ADR-0019) | INSERT `tool_events`, `usage`, `command_grains`, `subagent_transcripts`; UPDATE `audit.adr_count` |
